...
function Your_Games() {
   ...
   },
   {
    ...
    this.load_games = function(){
        ...
        cancel_game.load_display(this.complete_games[0].game_id)
    }
}

//Need to check if they already have an active game on there profile
function Cancel_Game() {
    this.load_display = function(game_selected_id) {        
        $('.gameinfobox').empty()
        let $gameinfobox = $('.gameinfobox')
        let $game_title = $('<h3>', {
            text: your_games.complete_games[game_selected_id].game_title
        })
        let $game_descr = $('<p>',{
            text: your_games.complete_games[game_selected_id].description,
        })
        let $game_date = $('<h6>', {
            text: `${your_games.complete_games[game_selected_id].game_date} ${your_games.complete_games[game_selected_id].game_time}`
        })
        let $cancel_btn = $('<a>', {
            href: '',
        ...
        $gameinfobox.append($game_title, '<hr>', $game_descr, $game_date, '<hr>', $cancel_btn);
